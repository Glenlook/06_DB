...
/* GROUP BY에 작성된 컬럼명만 SELECT 절에 작성할 수 있다!!! */
...
AND SALARY > ANY (SELECT SALARY -- 476만, 320만, 350만
              		FROM EMPLOYEE
              		JOIN JOB USING(JOB_CODE)
              		WHERE JOB_NAME = '과장');
...
WHERE SALARY > (
					SELECT AVG(SALARY) -- 서브쿼리 테이블 별칭이 "SUB"
					FROM EMPLOYEE "SUB"
...
SELECT ROWNUM || '등' 순위, "부서 코드", "부서명", "급여 평균"
FROM
		(SELECT 
...
SELECT 
    RANK() OVER(ORDER BY SALARY DESC) || '위' 순위,
...
SELECT 
    DENSE_RANK() OVER(ORDER BY SALARY DESC) || '위' 순위,
    EMP_NAME,
    SALARY
FROM EMPLOYEE;
