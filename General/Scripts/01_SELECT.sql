...
-- 단, 컬럼명은 모두 별칭 사용
SELECT EMP_ID 사번, EMP_NAME 이름, SALARY "급여(원)", SALARY * 12 "연봉(급여*12)" 
FROM EMPLOYEE;
...
SELECT EMP_ID, EMP_NAME, EMP_ID || EMP_NAME -- 200선동일
FROM EMPLOYEE;
...
WHERE DEPT_CODE IS NULL; -- 아동운, 이오리 2행 조회
...
-------------------------------------------------------------
/* 컬럼명 NOT IN (값1, 값2, 값3, ...) */
-- 컬럼 값이 IN () 내에 존재하지 "않으면" TRUE --> 조회 결과에 포함(NULL 미포함)
-------------------------------------------------------------
SELECT EMP_ID, EMP_NAME, DEPT_CODE
FROM EMPLOYEE
